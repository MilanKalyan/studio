    ...
    <Card className="w-full max-w-md mx-auto animate-fade-in opacity-0 shadow-2xl border-primary/20 bg-card/90 backdrop-blur-sm">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              required
              className={`bg-muted/30 focus:bg-background ${password !== confirmPassword && confirmPassword ? 'border-destructive ring-destructive focus-visible:ring-destructive' : ''}`}
              disabled={isLoading}
            />
          ...
          <Button type="submit" className="w-full retro-glow" disabled={isLoading || password !== confirmPassword}>
            {isLoading ? (
      ...
      <CardFooter className="text-center text-sm">
        <p className="text-muted-foreground w-full">
          ...
          <Link href="/auth/login" className="text-primary hover:underline" tabIndex={isLoading ? -1 : 0}>
            Log In
          </Link>
        </p>
      </CardFooter>
